...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.collider.CompareTag("OBS"))
        {
            if (isControlsInverted)
            {
                // Si ya estaban invertidos, muere el jugador
                animator.SetBool("DEAD", true);
            }
            else
            {
                // Si no estaban invertidos, invertir los controles
                isControlsInverted = true;
                invertedControlsTimer = invertedControlsDuration;  // Iniciar el temporizador
            }
        }
